...
class _ArticleSectionState extends State<ArticleSection> {
  ...
  void initState(){
    ...
    technologyArticles = Services.where((i) => i.type ==('Technology')).toList();
  ...
  Widget build(BuildContext context) {
    return Column(children: <Widget>[
      ...
      SingleChildScrollView(
        ...
        child:Row(
          children: (widget.cardData.type=='Recent')?recentArticles.map(
            (artData) => ArticleCard(artData: artData),
          ).toList()
          :(widget.cardData.type=='Technology')?technologyArticles.map(
